...
const roleIcons = {
    'Frontend Developer': <Code className="h-6 w-6 text-accent-secondary" />,
    'Backend Developer': <Server className="h-6 w-6 text-accent-primary" />,
    'AI Engineer': <BrainCircuit className="h-6 w-6 text-accent-primary" />,
    'Data Scientist': <BarChart className="h-6 w-6 text-accent-secondary" />,
    'DevOps Engineer': <Bot className="h-6 w-6 text-accent-primary" />,
    'QA Engineer': <TestTube2 className="h-6 w-6 text-accent-secondary" />,
    'UI/UX Designer': <Palette className="h-6 w-6 text-accent-secondary" />,
    'Mobile Developer': <Smartphone className="h-6 w-6 text-accent-primary" />,
};
...
export default function FormPage() {
  ...
  return (
    <div className="min-h-screen bg-background-primary text-primary flex flex-col items-center p-4 sm:p-8 font-sans">
       ...
       <main className="w-full max-w-7xl">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-12">
              {/* --- Step 1: Profile & Role --- */}
              <div>
                <h2 className="h2 font-bold flex items-center gap-3"><span className="text-accent-secondary font-black text-3xl">1</span> Your Profile & Target Role</h2>
                <div className="grid md:grid-cols-2 gap-8 mt-4">
                  <FormField control={form.control} name="githubUrl" render={({ field }) => ( <FormItem> <FormLabel>GitHub Profile URL</FormLabel> <div className="relative"> <Github className="absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-secondary" /> <FormControl><Input placeholder="https://github.com/your-username" {...field} className="pl-10"/></FormControl> </div> <FormMessage /> </FormItem> )}/>
                      ...
                      <FormControl> 
                        <div>
                          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3"> 
                            {jobRoles.map((role) => ( 
                              <button type="button" key={role} onClick={() => field.onChange(role)} className={`p-3 rounded-lg border-2 text-center transition-all duration-200 flex flex-col items-center justify-center gap-2 h-28 ${field.value === role ? 'bg-accent-secondary/10 border-accent-secondary shadow-lg' : 'bg-background-card border-[rgba(255,255,255,0.08)] hover:border-accent-secondary/60'}`}> 
                          ...
                          <div className="relative my-4 flex items-center">
                            ...
                            <span className="flex-shrink mx-4 text-secondary text-xs">OR</span>
                            <div className="flex-grow border-t border-[rgba(255,255,255,0.08)]"></div>
                          </div>
                          <Input 
              ...
              <div className="space-y-8">
                <h2 className="h2 font-bold flex items-center gap-3"><span className="text-accent-secondary font-black text-3xl">2</span> Showcase Your Work</h2>
                {/* Skills Section */}
                <FormField control={form.control} name="skills" render={() => ( <FormItem> <FormLabel className="text-lg">Your Skills</FormLabel> <Card className="card-glass"> <CardContent className="p-4"> <div className="flex flex-wrap gap-2 min-h-[4rem]"> {currentSkills.length > 0 ? ( currentSkills.map((skill) => ( <Badge key={skill} variant="secondary" className="text-base px-3 py-1"> {skill} <button type="button" onClick={() => handleRemoveSkill(skill)} className="ml-2 text-secondary hover:text-primary"> <XCircle className="h-4 w-4" /> </button> </Badge> )) ) : ( <p className="text-sm text-secondary p-3">Add skills using the input below.</p> )} </div> <div className="flex gap-2 mt-4"> <Input placeholder="e.g., React, Python, SQL" value={customSkill} onChange={(e) => setCustomSkill(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddCustomSkill(); }}}/> <Button type="button" variant="outline" onClick={handleAddCustomSkill}>Add Skill</Button> </div> </CardContent> </Card> <FormMessage /> </FormItem> )}/>
                {/* Projects Section */}
                  ...
                  <FormItem>
                    ...
                    <div className="space-y-4">
                      {fields.map((field, index) => (
                        <Card key={field.id} className="card-glass relative p-4 pr-12">
                           ...
                           <Button type="button" variant="ghost" size="icon" className="absolute top-2 right-2 text-secondary hover:text-red-500" onClick={() => remove(index)}> <Trash2 className="h-5 w-5" /> </Button>
                        </Card>
                      ))}
                      {fields.length < 3 && (
              ...
              <div className="text-center pt-8 border-t border-[rgba(255,255,255,0.08)]">
                  <h2 className="h2 font-bold flex items-center gap-3 justify-center"><span className="text-accent-secondary font-black text-3xl">3</span> Get Your Analysis</h2>
                  <p className="text-secondary mt-1 mb-6">Let's see what our AI career coach has to say.</p>
